...
def proc(
        ...
        clf_weights=ROOT / 'svm/finalized_model.sav',
        transform=ROOT / 'transform/std_scaler.bin',
...
#########################################
    ...
    for path, im, im0s, vid_cap, s in dataset:
        ...
        for i, det in enumerate(pred):  # per image
            ...
            _data = json.dumps({"counter":density})
            yield f"id: 1\ndata: {_data}\nevent: online\n\n"
            # socketio.emit('newnumber', {'number': density}, namespace='/test')
            # socketio.sleep(1)
